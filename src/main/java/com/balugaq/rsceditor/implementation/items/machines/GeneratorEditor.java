...
public class GeneratorEditor extends AbstractContainer {
    private static final MenuMatrix matrix = new MenuMatrix()
            ...
            .addItem("G", Icons.build_generator);

    public GeneratorEditor(@NotNull SlimefunItemStack item) {
        super(item);
    }
        ...
        return new BlockMenuPreset(getId(), getItemName()) {
            ...
            public void newInstance(@NotNull BlockMenu menu, @NotNull Block b) {
                ...
                menu.addMenuClickHandler(matrix.getChar("G"), (p, s, i, a) -> {
                    ...
                    Pair<Boolean, Integer> p5 = ItemUtil.isInteger(menu, matrix, "o");
                    if (!p5.getFirstValue()) {
                        p.sendMessage("You haven't set the energy production of the item");
                        return false;
                    }
                    int production = p5.getSecondValue();
                    writer.set("production", production);

                    Pair<Boolean, List<MachineRecipe>> p6 = ItemUtil.isMachineRecipes(menu, matrix, "m");
                    if (p6.getFirstValue()) {
                        for (MachineRecipe recipe : p6.getSecondValue()) {
                            String recipe_key = "fuels." + recipe.getName();
                            writer.set(recipe_key + ".item", recipe.getInputs()[0]);
                            writer.set(recipe_key + ".seconds", recipe.getProcessingTime());
                            writer.set(recipe_key + ".chooseOne", recipe.isChooseOne());
                            writer.set(recipe_key + ".forDisplay", recipe.isForDisplay());
                            writer.set(recipe_key + ".hide", recipe.isHide());
                            writer.set(recipe_key + ".output", recipe.getOutputs()[0]);
                        }
                    }
                    ...
                    ClipboardUtil.send(p, "Menu Editor: ", writer.toString());
